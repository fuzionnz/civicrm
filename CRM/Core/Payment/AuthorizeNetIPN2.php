 ...
 *
 */
class CRM_Core_Payment_AuthorizeNetIPN2 extends CRM_Core_Payment_BaseIPN {
  function __construct() {
    parent::__construct();
  }
/**
 * Main function called from IPN
 *
 * We call extract input & ids from $_POST & then call process input
 *
 ...
 */
  function main($component = 'contribute') {
    ...
    if ($x_subscription_id) {
      ...
      $this->getIDsFromInput($ids, $input);
      return $this->processInput($input, $component);
    }
  }

/**
 * This is the portion of the 'main' function that happens after input is extracted from POST
 * so that the function can be called & tested without faking $_POST
 *
 * @param array $input
 * @param string $component
 */
  function processInput($input, $component = 'contribute'){
    $this->calculateIDs($ids, $input);
...
/**
 * Extract input parameters from POST var
 * @todo function rationale for received ids array is a bit ?? it is empty when passed to getBillingId
 * @param array $input
 * @param array $ids
 * @return boolean
 */
  ...
  /**
   * get those IDS that are in the POST array
   * @param array $ids
   * @param array $input
   */
  function getIDsFromInput(&$ids, &$input) {
    $ids['contact'] = self::retrieve('x_cust_id', 'Integer');
    $ids['contribution'] = self::retrieve('x_invoice_num', 'Integer');
  }

/**
 * Populate those IDS that are calculated from other variables rather than $_POST data
 * @param array $ids
 * @param array $input
 */
  function calculateIDs(&$ids, &$input){
...
/**
 * retrive data from POST array
 * @todo this seems to duplicate a core function for no obvious reason
 * @param string $name
 * @param string $type
 * @param boolean $abort
 * @param array $default
...
/**
 * Check MD 5 hash for recurring contributions
 * @param array $ids
 * @param array $input
 * @return boolean
 */
